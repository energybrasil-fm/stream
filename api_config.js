...
// ==========================================
// SISTEMA CALLBACK JSONP (FALLBACK PARA 100HITZ)
// ==========================================
function fetch100HitzJSONP(url) {
    return new Promise((resolve, reject) => {
        // Gera um nome único para a função de callback
        const callbackName = 'jsonp_callback_' + Math.round(1000000 * Math.random());
        
        // Usa o endpoint GET do allorigins que envelopa o texto no Callback JSONP bypassando CORS
        const proxyUrl = `https://api.allorigins.win/get?url=${encodeURIComponent(url)}&callback=${callbackName}`;
        
        // Define a função de callback no escopo global
        window[callbackName] = function(data) {
            delete window[callbackName];
            document.body.removeChild(script);
            if (data && data.contents) {
                resolve(data.contents); // 'contents' guarda o texto "Artista - Título"
            } else {
                reject(new Error("Resposta vazia no callback"));
            }
        };
...
// ==========================================
// SISTEMA NATIVO DE FETCH PARA APIS JSON
// ==========================================
const fetchCache = {};
...
async function safeFetchJson(url) {
    ...
    const fetchPromise = (async () => {
        // Faz a requisição de forma nativa e limpa, sem usar arrays de proxy (Removidos)
...
export async function fetchStationData(station) {
    ...
    try {
        ...
        // ==============================================================
        // LÓGICA ESPECIAL PARA TEXT/PLAIN (100HITZ / GOTRADIO) COM CALLBACK
        // ==============================================================
        if (station.type === '100hitz') {
            ...
            try {
                ...
                rawTitle = await response.text();
            } catch (err) {
                // Se bloquear por CORS, usa o fallback do Callback em JSONP
                try {
                    rawTitle = await fetch100HitzJSONP(timestampedUrl);
                } catch (fallbackErr) {
                    console.error("100hitz Callback fetch error:", fallbackErr);
                }
            }
        ...
        // ==============================================================
        // TODAS AS DEMAIS APIS JSON (SEM PROXIES CORS)
        // ==============================================================
